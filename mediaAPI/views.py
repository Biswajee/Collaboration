...
def upload_image(request):
    ...
    return render(request, "mediaAPI/index.html", {"form": form})
...
class Api(APIView):
    ...
    def get_by_id(request):
        ...
        if query is None:
            context = {"error": "You need to pass in parameters"}
            return JsonResponse(context)
        else:
    ...
    def delete_by_id(request):
        ...
        if query is None:
            ...
            query_set = images.objects.get(id=query)
            query_set.delete()
            context = {"id": query, "status": "deleted"}
            return JsonResponse(context)
    ...
    def update_image(request):
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if query is None:
                    ...
                    for filename, file in request.FILES.items():
                        ...
                        images.objects.filter(id=query).update(
                            image="ImageAPI/" + name
                        )
                    context = {
                        "id": query,
            ...
            form = image_update()
        return render(request, "mediaAPI/update.html", {"form": form})
